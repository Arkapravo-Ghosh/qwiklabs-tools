...
const typedAssignments: string[] = assignments;
...
const main = async (): Promise<void> => {
  ...
  for (const profile of data) {
    ...
    const scraped_data: ProfileData = {
      ...
      incomplete_assignments_count: assignments.length - badges.length,
      completed_assignments_count: badges.filter((badge) => typedAssignments.includes(badge)).length,
      badges: badges,
      incomplete_assignments: assignments.filter((assignment) => !badges.includes(assignment)),
